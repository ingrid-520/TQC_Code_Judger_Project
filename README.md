# TQC Python 練習系統 (TQC Code Judger)

一個在瀏覽器端執行 Python 的 TQC+ 程式語言 Python 練習平台與程式碼測試系統 (Code Judger)。
與傳統的 Code Judger 不同，本系統無需安裝任何後端環境（如 Flask 或 Django），而是透過前端技術直接在您的瀏覽器沙盒中執行 Python，開箱即用。
...
> ⚠️ **已知限制**：提交後，若要修改程式碼，需重新整理頁面（Ctrl + F5）。
